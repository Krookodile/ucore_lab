...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011293 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if (blkoff != 0)//读取的开始位置不是对其某个block的开始处
    {
        ...
        if (ret != 0)
            goto out;
        alen += size;//更新已经读取长度alen
        if (nblks == 0) //读取完了，跳到out部分
            goto out;
    ...
    while(nblks != 0)
    {
    	ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);//得到实际的磁盘编号，操作失败跳到out部分
    	if (ret != 0)
    		goto out;
    	ret = sfs_buf_op(sfs, buf, size, ino, blkoff);//进行实际的读操作，操作失败跳到out部分
    	if (ret != 0)
    		goto out;
    ...
    //(3)结束地址不对其，有剩余部分需要读取
    size = endpos % SFS_BLKSIZE;//得到剩余读取部分的长度
    if(size != 0)
    {
    	ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);//得到实际的磁盘编号，操作失败跳到out部分
    	if (ret != 0)
